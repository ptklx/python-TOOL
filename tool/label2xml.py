...
def born_xml(file_path, img, x_list, label_list, save_dir):
    """
    ...
    # text = file_path.split('/')[-2]
    text = file_path.split('\\')[-2]  #windows
    folder_text = doc.createTextNode(text)
    ...
    text = file_path.split('\\')[-1]
    filename_text = doc.createTextNode(text)
    filename.appendChild(filename_text)
    annotation.appendChild(filename)

    # path
    path = doc.createElement('path')
    path_text = doc.createTextNode(file_path)
    path.appendChild(path_text)
    annotation.appendChild(path)

    # source
    source = doc.createElement('source')
    source_text = doc.createTextNode('Unknown')
    ...
    depth_text = doc.createTextNode(str(img.shape[2]))  # 元素内容写入
    ...
    for index in range(len(label_list)):
        ...
        xmin_text = doc.createTextNode(str(int(x[0])))
        xmin.appendChild(xmin_text)

        ymin = doc.createElement('ymin')
        ymin_text = doc.createTextNode(str(int(x[1])))
        ymin.appendChild(ymin_text)

        xmax = doc.createElement('xmax')
        xmax_text = doc.createTextNode(str(int(x[2])))
        xmax.appendChild(xmax_text)

        ymax = doc.createElement('ymax')
        ymax_text = doc.createTextNode(str(int(x[3])))
    ...
    # f = open(save_dir + '/' + file_path.split('/')[-1].split('.')[0] + '.xml', 'a')
    f = open(save_dir + '\\' + file_path.split('\\')[-1].split('.')[0] + '.xml', 'w')
    doc.writexml(f, addindent=' ', newl='\n')
    f.close()


def self_define_rectangle(img, c1, c2, color, tl):
    # 自定义矩形框
    tf = max(tl - 1, 1)  # font thickness
    cv2.line(img, c1, (c1[0] + 30, c1[1]), color, thickness=tf, lineType=cv2.LINE_AA)
    cv2.line(img, c1, (c1[0], c1[1] + 30), color, thickness=tf, lineType=cv2.LINE_AA)

    cv2.line(img, (c1[0], c2[1]), (c1[0], c2[1] - 30), color, thickness=tf, lineType=cv2.LINE_AA)
    cv2.line(img, (c1[0], c2[1]), (c1[0] + 30, c2[1]), color, thickness=tf, lineType=cv2.LINE_AA)

    cv2.line(img, (c2[0], c1[1]), (c2[0] - 30, c1[1]), color, thickness=tf, lineType=cv2.LINE_AA)
    cv2.line(img, (c2[0], c1[1]), (c2[0], c1[1] + 30), color, thickness=tf, lineType=cv2.LINE_AA)

    cv2.line(img, c2, (c2[0], c2[1] - 30), color, thickness=tf, lineType=cv2.LINE_AA)
    cv2.line(img, c2, (c2[0] - 30, c2[1]), color, thickness=tf, lineType=cv2.LINE_AA)
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    self_define_rectangle(img, c1, c2, [255, 255, 255], tl)
    if label:
        tf = max(tl - 1, 1)  # font thickness
